...
```
-->
StochKit2R is available on github. You should be able to install it from the R console using:
```{r, eval=FALSE}
...
You may seem some compilation warning messages but no errors. After successful installation, load and attach the package with:
```{r, eval=FALSE}
library(StochKit2R)
```
