...
Network (https://cran.r-project.org/). Therefore, you should be able to install
...
https://github.com/kevinsanft/StochKit2R/releases. On Windows, download the .zip
...
call to `devtools::install_github("kevinsanft/StochKit2R")` to install
StochKit2R.) 
...
```r
install.packages("devtools") #run once on your system
devtools::install_github("kevinsanft/StochKit2R", ref="serial")
```
...
at https://www.kevinsanft.com/StochKit2R/, on Linux and Mac OS X by running the
...
https://clang-omp.github.io/, but I have not tested it.
...
or from source.
2.	Download StochKit2R_\<version\>.tar.gz from https://www.kevinsanft.com/StochKit2R/
...
1. Download the Mac clang binary with OpenMP support (as of this writing, the
   current file is
   https://mac.R-project.org/libs/clang-4.0.0-darwin15.6-Release.tar.gz)
   from https://mac.R-project.org/libs/.
...
StochKit2R_\<version\>.zip from https://www.kevinsanft.com/StochKit2R/.

To install the package from .zip file:

* In RStudio: under the Tools menu, choose "Package Archive File (.zip, .tar.gz)"
  from the "Install from:" dropdown menu. Then locate the .zip file, then click
  Install.
* In Rgui: under the Packages menu, choose "Install package(s) from local zip
  files". Then locate the .zip file.
-->
...
3. If you receive an error message reading "Error in fetch(key) : lazy-load
   ...
   (https://github.com/r-lib/devtools/issues/419). If you restart R and reload
