...
histogramDistance <- function(data1, species1, timeIndex1=NULL, data2, species2, timeIndex2=NULL) {
	...
	if (!(is.integer(species1) || is.numeric(species1) || is.character(species1))) {
	...
	if (is.integer(species1) || is.numeric(species1)) {
	...
	if (is.null(timeIndex1)) {
		...
		if (!is.integer(timeIndex1) && !is.numeric(timeIndex1)) {
	...
	if (!(is.integer(species2) || is.numeric(species2) || is.character(species2))) {
	...
	if (is.integer(species2) || is.numeric(species2)) {
		# convert to species name
	...
	if (is.null(timeIndex2)) {
		...
		if (!is.integer(timeIndex2) && !is.numeric(timeIndex2)) {
	...
	p <- ggplot(
		...
		fill="red",
		alpha=.65
	) + ylab("Bin Counts") + xlab("Bin Centers") + ggtitle(paste("Species:", sID1, " Time:", time1))
	...
	p3 <- p2 + annotate(
		"text",
		size=4,
		x=(dataRange[length(dataRange)] + dataRange[1]) / 2,
		y=maxCount * 1.1,
		label=sprintf("Euclidean d=%2.3f, Manhattan d=%2.3f", euclidean_distance, manhattan_distance)
	)
